...
### Methodology
The methodology used in this project is the waterfall development. Before going to the next part of the project the activity before has to be finished within reason. The documentation is not included.

The stress on a society will be viewed as the burden on the workforce of people who are older than 65. The stress can be viewed in the GDP per capita, healtcare costs, and the age dependency young (burden on the workforce of people younger than 65).
...
| ------------- |:-------------:| -----:|
| Age dependency old    | right-aligned | $1600 |
| Age dependency young      | centered      |   $12 |
| GDP per capita | are neat      |    $1 |
| Healtcare expenditure |
...
The data is collected from the world bank site. This site gives access to open data, which can be downloaded as a csv file. With the following reddit code the different files are made into a database. After that they're all given the same header and cleaned up a bit. Only the country files are saved and empty columns are cleared.
Before a selection was made which parameters to include, similar data was acquired. After some initial exploration some data was dropped.

### Description

The data is devided into deferent dataframes. Every dataframe has one country per row. However, among the countries there are also aggregated areas, like the arab world. The first for columns are the name of the country, the abbreviation of the country, the indicator name, and the indicator code. Thereafter ever year between 1960 and 2017 has a seperate column.

### Quality Discussion

The quality of the data was less than expected. There were a lot of missing values, even up until the last couple of years. This has great influences on the research done and the gotten conclusion.
